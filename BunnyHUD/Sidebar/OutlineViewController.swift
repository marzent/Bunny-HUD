...
{ // To enable/disable menu items for the outline view.
    ...
    private func newOverlayWindow(node: Node) {
        if node.isDirectory {
            ...
            let overlayWinController = storyboard!.instantiateController(withIdentifier: "OverlayWindow") as! NSWindowController
    ...
    func removeWindow(item: Node) {
        if item.isDirectory {
            ...
            overlayControllers[item.identifier]?.view.window?.close()
        }
    }
...
//        addNode(node)
    ...
    private func setupObservers() {
        ...
        treeControllerObserver =
            treeController.observe(\.selectedObjects, options: [.new]) { treeController, _ in
